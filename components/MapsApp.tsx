...
import eventsData from "@/constants/historyEvents";
...
const MapsApp = () => {
  ...
  const [favourites, setFavourites] = useState<number[]>(() => {
    const savedFavorties = localStorage.getItem("favourites");

    return savedFavorties ? JSON.parse(savedFavorties) : [];
  });

  const handleFavourtiteClick = (eventId: number) => {
          ...
          {activeEvent && (
            <Popup position={activeEvent.position}>
              ...
              <button
                ...
                onClick={() => handleFavourtiteClick(activeEvent.id)}
              >
